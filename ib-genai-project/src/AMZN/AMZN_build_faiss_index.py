...
# chunk .txt file produced by the AMZN_chunk_all_*.py /
# AMZN_build_cross_statement_relationships_files.py scripts.
#
# NOTE: the original AAPL_build_faiss_index.py had two path bugs -
# "ratios" pointed at "ratio_all_chunks.txt" (missing the "s") and
# "relationships" pointed inside the "ratios" folder instead of its own
# "relationships" folder. Both are corrected below to match what the
# AMZN_chunk_all_growth_keymetrics_ratios_files.py and
# AMZN_build_cross_statement_relationships_files.py scripts actually
# write out (ratios_all_chunks.txt in /ratios, and
# relationships_all_chunks.txt in /relationships).
CHUNK_SOURCES: dict[str, Path] = {
    "balance_sheet": Path(
        r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
        r"\ib-genai-project\data\chunks\AMZN\balance_sheet"
        r"\balance_sheet_all_chunks.txt"
    ),
    "cash_flow": Path(
        r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
        r"\ib-genai-project\data\chunks\AMZN\cash_flow"
        r"\cash_flow_all_chunks.txt"
    ),
    "income_statement": Path(
        r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
        r"\ib-genai-project\data\chunks\AMZN\income_statement"
        r"\income_statement_all_chunks.txt"
    ),
    "growth": Path(
            r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
            r"\ib-genai-project\data\chunks\AMZN\growth"
            r"\growth_all_chunks.txt"
        ),
    "key_metrics": Path(
                r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
                r"\ib-genai-project\data\chunks\AMZN\key_metrics"
                r"\key_metrics_all_chunks.txt"
            ),
     "ratios": Path(
                    r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
                    r"\ib-genai-project\data\chunks\AMZN\ratios"
                    r"\ratios_all_chunks.txt"
                ),
     "relationships": Path(
                         r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
                         r"\ib-genai-project\data\chunks\AMZN\relationships"
...
OUTPUT_FOLDER = Path(
    r"C:\AZ_DEVOPS_PYTHON\Investment_Banking"
    r"\ib-genai-project\data\faiss_db\AMZN"
)

FAISS_INDEX_FILE = OUTPUT_FOLDER / "amzn_financials.index"
METADATA_FILE = OUTPUT_FOLDER / "amzn_financials_metadata.pkl"
METADATA_JSON_PREVIEW_FILE = OUTPUT_FOLDER / "amzn_financials_metadata_preview.json"
...
def main() -> None:
    ...
    sample_query = "What was Amazon's total current assets?"
