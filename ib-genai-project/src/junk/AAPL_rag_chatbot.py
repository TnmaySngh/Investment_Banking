...
print("\nIncome Statement Collections:")
for c in incomestatement_collections:
    print(" -", c.name)
...
if len(incomestatement_collections) == 0:
    raise Exception("No collection found in Income Statement database.")
...
incomestatement = incomestatement_client.get_collection(incomestatement_collections[0].name)
...
def ask(question):
    ...
    incomestatement_result = retrieve(incomestatement, question)
    ...
    if incomestatement_result["documents"]:
        documents.extend(incomestatement_result["documents"][0])
