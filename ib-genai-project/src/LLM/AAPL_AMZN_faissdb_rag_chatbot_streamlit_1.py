...
- A company switcher (AAPL / AMZN / Both) in the sidebar. Switching
  companies swaps the FAISS index/metadata and clears chat history +
  memory, so conversations never mix across an unrelated switch.
- A "Both" (cross-company) mode that loads the AAPL and AMZN FAISS
  indexes side by side, retrieves top_k chunks from EACH company for
  every question, tags every chunk with its source company, and feeds
  the combined, company-labeled context to a single LLM call - so you
  can ask direct comparison questions ("Compare AAPL and AMZN gross
  margin trends") and get one answer grounded in both companies' data.
...
COMPANIES = ["AAPL", "AMZN", "BOTH"]
COMPANY_DEFAULT = "AAPL"

COMPANY_LABELS = {
    "AAPL": "Apple Inc. (AAPL)",
    "AMZN": "Amazon.com, Inc. (AMZN)",
    "BOTH": "\U0001F500 Apple + Amazon (Cross-Company)",
}

# Single-company codes only (excludes the synthetic "BOTH" entry) - used
# whenever we need to actually load/iterate real FAISS indexes.
SINGLE_COMPANIES = [c for c in COMPANIES if c != "BOTH"]

# Chip color per company, used to visually tell AAPL vs AMZN sources
# apart once both are retrieved side by side in "Both" mode.
COMPANY_COLORS = {
    "AAPL": "#2f6fed",
    "AMZN": "#e08a2b",
}
...
def build_context_and_sources(question, client, index, records, top_k, statement_type_filter, company_label=None):
    """Run FAISS search and shape results into a context string + source list.

    company_label is stamped onto every chunk (both in the text handed to
    the LLM and in the source metadata shown in the UI) so that, when
    combined with chunks from another company via
    build_context_and_sources_multi, the model and the user can always
    tell which company a given piece of evidence belongs to.
    """
...
def build_context_and_sources_multi(question, client, company_data, top_k, statement_type_filter):
    """Cross-company retrieval for "Both" mode.

    Runs an independent FAISS search against EACH company's index (so
    AMZN chunks can never crowd out AAPL chunks, or vice versa, just
    because one company's data happens to score higher), tags every
    chunk with its source company, then merges everything into a single
    context string + source list for one combined LLM call.

    company_data: dict mapping company code ("AAPL", "AMZN") -> (index, records).
    """
    context_parts = []
    all_sources = []

    for comp, (comp_index, comp_records) in company_data.items():
        if comp_index is None or not comp_records:
            continue
        comp_context, comp_sources = build_context_and_sources(
            question, client, comp_index, comp_records, top_k, statement_type_filter, company_label=comp
        )
        if comp_context:
            context_parts.append(comp_context.strip("\n"))
        all_sources.extend(comp_sources)

    # Re-sort the merged list so the overall best evidence (regardless of
    # which company it came from) surfaces first in the UI.
    all_sources.sort(key=lambda s: (s["similarity"] if s["similarity"] is not None else -1), reverse=True)
...
Each context chunk is tagged with a "Company:" field (AAPL and/or AMZN).
If chunks from both companies are present, treat them as two separate
sets of facts - never blend or average a figure from one company with a
figure from the other. When asked to compare the two, address AAPL and
AMZN as distinct entities, cite each company's own numbers explicitly,
and only draw a comparison once both companies' relevant figures are
individually grounded in their own tagged context.
...
def format_transcript(messages, company):
    label = "AAPL + AMZN (Cross-Company)" if company == "BOTH" else company
    lines = [f"# {label} Research Assistant — Conversation Export", f"_Exported {datetime.now().strftime('%Y-%m-%d %H:%M')}_", ""]
    for msg in messages:
...
with st.sidebar:
    ...
    if company == "BOTH":
        # Cross-company mode: load AAPL's and AMZN's indexes side by side.
        # company_data feeds build_context_and_sources_multi below, which
        # queries each index independently and merges the tagged results
        # into one context for a single LLM call.
        company_data = {}
        for comp in SINGLE_COMPANIES:
            comp_paths = get_company_paths(comp)
            comp_index, comp_records, comp_error = load_index_and_records(comp)
            company_data[comp] = (comp_index, comp_records)
            if comp_index is not None:
                st.success(
                    f"\U0001F4C8 **{comp} FAISS Index**  \n`{comp_paths['index']}` · {len(comp_records)} chunks",
                    icon="✅",
                )
            else:
                st.error(f"\U0001F4C8 **{comp} FAISS Index**  \n{comp_error or 'Unavailable'}", icon="⚠️")

        indexes_ready = all(v[0] is not None for v in company_data.values())
        records = [r for _, comp_records in company_data.values() for r in comp_records]
        index = None  # no single index in Both mode; company_data holds both
    else:
        company_paths = get_company_paths(company)
        index, records, index_error = load_index_and_records(company)
        company_data = {company: (index, records)}
        indexes_ready = index is not None
...
if company == "BOTH":
    st.title("Apple vs Amazon Financial Statement Assistant")
    st.caption(
        "Ask cross-company questions comparing AAPL and AMZN across Balance Sheet, Cash Flow, "
        "Income Statement, Growth, Key Metrics, Ratios, and Cross-Statement Relationships data. "
        "Each answer is grounded in chunks retrieved from BOTH companies' filings, tagged by source company."
    )
else:
...
if not st.session_state.messages:
    ...
    if company == "BOTH":
        examples = [
            "Compare AAPL and AMZN's revenue and net income for FY2025.",
            "Whose gross margin improved more from FY2024 to FY2025 — Apple or Amazon?",
            "How do AAPL and AMZN's free cash flow trends compare?",
        ]
    else:
        examples = [
...
def render_sources(sources, key_prefix="", query=""):
    with st.expander(f"📎 {len(sources)} source{'s' if len(sources) != 1 else ''} used"):
        for i, src in enumerate(sources):
            ...
            company_chip = ""
            src_company = src.get("company")
            if src_company:
                company_color = COMPANY_COLORS.get(src_company, "#7a7f8c")
                company_chip = (
                    f"<span class='source-chip' style='border-color:{company_color};"
                    f"color:{company_color};font-weight:600;'>{src_company}</span>"
                )
...
# Chat input
chat_placeholder = "Ask a comparison question about AAPL and AMZN..." if company == "BOTH" else f"Ask about {company}'s financials..."
...
def run_query(question_text, is_regenerate=False):
    ...
    with st.chat_message("assistant", avatar=ASSISTANT_AVATAR):
        ...
        if not indexes_ready:
            ...
            try:
                with st.spinner(
                    "Retrieving relevant filings data from both companies..." if company == "BOTH"
                    else "Retrieving relevant filings data..."
                ):
                    if company == "BOTH":
                        context, sources = build_context_and_sources_multi(
                            question_text, embedder, company_data, top_k, statement_type_filter
                        )
                    else:
